...
@pytest.mark.anyio
async def test_look_self_true_id_uses_legacy_attnam_matching_when_transformed():
    player = _build_player(
        plyrid="necro",
    ...
    assert "WILDES" not in message_ids


@pytest.mark.anyio
async def test_look_self_uses_transformed_attention_name():
    player = _build_player(
        plyrid="necro",
        altnam="Some willowisp",
        attnam="willowisp",
    ...
    result = await dispatcher.dispatch("look", {"raw": "willowisp"}, state)
