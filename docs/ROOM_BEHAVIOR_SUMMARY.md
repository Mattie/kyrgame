...
- **Room 19 — `fthick` (flaming thicket):** Walking into thicket triggers burn text, broadcasts the burning misstep to others, and applies 10 HP death-checked damage to the actor.【F:legacy/KYRROUS.C†L642-L656】
- **Room 20 — `rubies` (ruby cache):** Getting a ruby can grant a ruby object 20% of the time if inventory space allows; failure or a full pack produces failure text and applies 8 HP death-checked damage to the actor.【F:legacy/KYRROUS.C†L576-L592】
...
- **Room 38 — `magicf` (Fountain of Eternal Magic):** Speaking `FOUNTI` grants `BLESSD`; `drop`/`throw`/`toss <inventory item> in fountain` consumes the item. Blessed pinecones advance the scroll counter and every third one places scroll object 35 in a random room with capacity; shards grant object 16 every sixth offering; other gifts emit the generic acceptance text. Reference: `legacy/KYRROUS.C:759-819`.
