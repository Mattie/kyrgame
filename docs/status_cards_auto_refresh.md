...
4. **Auto-refresh scheduling**
   ...
   - Start a 2-second interval while connected that fires silent refresh commands for active auto-refresh cards; stop when disconnected or no cards are initialized.
...
- Added silent auto-refresh scheduling after every manual command and on a 2-second interval while connected, reusing the card’s command verb and tagging requests with `status_card` metadata for traceability.
