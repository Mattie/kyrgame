...
- [x] Added guarded moving-mob singleton audit/cleanup tooling for dryad and Zar room-object drift, exposed singleton diagnostics in the admin mob tracker, blocked admin drops of singleton mob objects, and hardened dryad/Zar relocation against orphaned copies.
...
- [x] Preserved silver altar birthstone and brown-gem stump progress on wrong or premature offerings while still consuming valid offered inventory objects, matching `silver()`/`stumpr()` reset boundaries in `legacy/KYRROUS.C`. Room 18 treats modern active sessions as always satisfying the legacy `chklvl(6)`/`KYRKEY` gate, so the old missing-key reset branch is obsolete in the port.
